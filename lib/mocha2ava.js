...
function insertRequires (root, j) {
  let avaImported = false;
  root.find(j.VariableDeclaration)
    .filter(p => p.value.declarations.filter(d =>
      d.type === 'VariableDeclarator' && d.id.type === 'Identifier' && d.id.name === 'test' &&
      d.init.type === 'CallExpression' && d.init.callee.type === 'Identifier' && d.init.callee.name === 'require' &&
      d.init.arguments.filter(arg => arg.type === 'StringLiteral' && arg.value === 'ava').length > 0).length > 0)
    .forEach(() => avaImported = true)
  root.find(j.ImportDeclaration)
    .filter(p => p.value.specifiers.filter(s => s.type === 'ImportDefaultSpecifier' && s.local.type === 'Identifier' && s.local.name === 'test').length > 0)
    .filter(p => p.value.source.type === 'StringLiteral' && p.value.source.value === 'ava')
    .forEach(() => avaImported = true)
  ...
  if (!avaImported) {
    root.find(j.Program)
      .forEach(p => {
        ...
        const useStrictIdx = body.findIndex(n => n.type === 'ExpressionStatement' && n.expression.type === 'StringLiteral' && n.expression.value === 'use strict')
